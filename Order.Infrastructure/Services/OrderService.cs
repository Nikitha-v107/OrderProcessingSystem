...
namespace Order.Infrastructure.Services
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<Order.Domain.Models.Order?> GetOrderByIdAsync(string id)
        {
            _logger.LogInformation("Attempting to retrieve order by ID: {OrderId}", id);
            ...
            catch (Exception ex)
            {
                ...
                throw new InfrastructureException($"Failed to retrieve order with ID: {id} due to an infrastructure issue.", ex);
            }
        }
