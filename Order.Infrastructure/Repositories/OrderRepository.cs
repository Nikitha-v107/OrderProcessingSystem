...
namespace Order.Infrastructure.Repositories
{
    public class OrderRepository : IOrderRepository
    {
        ...
        {
            _logger.LogInformation("Attempting to retrieve order by ID: {OrderId} from Cosmos DB.", id);
            try
            {
                ...
                _logger.LogInformation("Order ID: {OrderId} retrieved successfully from Cosmos DB.", id);
                return response.Resource;
            }
            catch (CosmosException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
                _logger.LogWarning("Order ID: {OrderId} not found in Cosmos DB.", id);
                return null;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error retrieving order ID: {OrderId} from Cosmos DB.", id);
                throw new InfrastructureException($"Failed to retrieve order ID: {id} from Cosmos DB.", ex);
            }
        }
        ...
        {
            _logger.LogInformation("Attempting to update status of order ID: {OrderId} to {Status} in Cosmos DB.", id, status);
            try
            {
                ItemResponse<Order.Domain.Models.Order> response = await _container.ReadItemAsync<Order.Domain.Models.Order>(id, new PartitionKey(id));
                ...
                if (order == null)
                {
                    _logger.LogWarning("Order ID: {OrderId} not found for status update in Cosmos DB.", id);
                    throw new NotFoundException($"Order with ID: {id} not found for status update.");
                }
                order.Status = status;
                await _container.ReplaceItemAsync(order, id, new PartitionKey(id));
                _logger.LogInformation("Order ID: {OrderId} status updated to {Status} successfully in Cosmos DB.", id, status);
            }
            catch (CosmosException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
                _logger.LogWarning("Order ID: {OrderId} not found for status update in Cosmos DB.", id);
            ...
            {
                _logger.LogError(ex, "Error updating status for order ID: {OrderId} to {Status} in Cosmos DB.", id, status);
                throw new InfrastructureException($"Failed to update status for order ID: {id} in Cosmos DB.", ex);
            }
        }
    }
}
